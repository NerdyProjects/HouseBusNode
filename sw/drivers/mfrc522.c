 ...
 * |----------------------------------------------------------------------
 * |
 * | This driver has been slightly modified in November 2019 by Matthias Larisch
 */
#include "mfrc522.h"
#include "node.h"
...
void TM_MFRC522_Init(void) {
	TM_MFRC522_InitPins();
	spiStart(MFRC522_SPI, &ls_spicfg);
	//node_debug(LOG_LEVEL_DEBUG, "MFRC522", "Reset...");
...
TM_MFRC522_Status_t TM_MFRC522_Check(uint8_t* id) {
	...
	node_debug(LOG_LEVEL_DEBUG, "MFRC522", "Request...");
...
void TM_MFRC522_InitPins(void) {
	...
	/* SPI */
	palSetPadMode(GPIOB, 13, PAL_MODE_ALTERNATE(0) | PAL_STM32_OSPEED_LOWEST);
	palSetPadMode(GPIOB, 14, PAL_MODE_ALTERNATE(0) | PAL_STM32_OSPEED_LOWEST);
	palSetPadMode(GPIOB, 15, PAL_MODE_ALTERNATE(0) | PAL_STM32_OSPEED_LOWEST);
	/* CS */
	palSetPadMode(GPIOB, 12, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_LOWEST);
	/* Reset */
	palSetPadMode(GPIOA, 8, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_LOWEST);
	MFRC522_CS_HIGH;
	...
	palSetPad(GPIOA, 8);
	/* Wait for device becoming active again */
	chThdSleepMilliseconds(1);
}
...
void TM_MFRC522_WriteRegister(uint8_t addr, uint8_t val) {
	...
	MFRC522_CS_LOW;
	//Send address
	spiPolledExchange(MFRC522_SPI, (addr << 1) & 0x7E);
	// send val
	spiPolledExchange(MFRC522_SPI, val);
	//CS high
	MFRC522_CS_HIGH;
}
...
uint8_t TM_MFRC522_ReadRegister(uint8_t addr) {
	...
	spiPolledExchange(MFRC522_SPI, ((addr << 1) & 0x7E) | 0x80);
	val = spiPolledExchange(MFRC522_SPI, MFRC522_DUMMY);
	//CS high
	MFRC522_CS_HIGH;
	return val;	
}
